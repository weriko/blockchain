    ...
    def load(self, path):
        ...
        for i in range(len(blocks)-1):
            if hs.sha256(str(blocks[i]["data"] + str(blocks[i]["prev_hash"]) + str(blocks[i]["nonce"])+ str(blocks[i]["time"])).encode()).hexdigest() == blocks[i+1]["prev_hash"] :
                self.blocks.append(Block(blocks[i]["data"] ,blocks[i]["prev_hash"] , blocks[i]["hash"], blocks[i]["nonce"], blocks[i]["time"], blocks[i]["no"]))
            else:
                print("Error checking")
                print(i)
               
               
        self.blocks.append(Block(blocks[-1]["data"] ,blocks[-1]["prev_hash"] , blocks[-1]["hash"], blocks[-1]["nonce"], blocks[-1]["time"], blocks[-1]["no"]))
